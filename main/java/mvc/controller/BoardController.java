...
public class BoardController extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (command.equals("/BoardListAction.do")) {//등록된 글 목록 페이지 출력하기
			...
			rd.forward(request, response);
				...
				boolean result = requestBoardUpdate(request);
				...
				boolean result = requestBoardDelete(request);
			    RequestDispatcher rd;
			    if (result) {
			        rd = request.getRequestDispatcher("/BoardListAction.do");
			    } else {
   ...
   //새로운 글 등록하기
	public void requestBoardWrite(HttpServletRequest request){
		...
		board.setPasswd(request.getParameter("passwd")); // 비밀번호 추가
		board.setSubject(request.getParameter("subject"));
		board.setContent(request.getParameter("content"));	
		
		System.out.println(request.getParameter("name"));
		System.out.println(request.getParameter("passwd"));// 비밀번호 추가
        ...
        if (parentNum == null || parentNum.isEmpty()) {
            // 원글
            board.setRe_ref(0);
            board.setRe_lev(0);
            board.setRe_seq(0);
            ...
            dao.updateReSeq(re_ref, re_seq); // 기존 seq 밀기
	...
	public boolean requestBoardUpdate(HttpServletRequest request){
		...
		board.setContent(request.getParameter("content"));		
		 ...
		 if ("admin1".equals(sessionId) || inputPw.equals(dao.getPasswdByNum(num))) {
	...
	//선택된 글 삭제하기
	public boolean requestBoardDelete(HttpServletRequest request){
